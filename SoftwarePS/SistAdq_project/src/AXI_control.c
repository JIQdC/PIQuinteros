...
int inputDelaySet_data(uint8_t i, uint8_t taps)
{
    ...
    //assert DELAY CTRL is locked for corresponding FPGA bank
    if (g_adcPinPositions[i]) locked_addr = DELAY_BASE_ADDR + LOCKED13_OFF;
    ...
    //set input delay values
    wr_val = taps;
    memwrite(DELAY_BASE_ADDR + DATA_IN_OFF + 4 * i, &wr_val, 1);

    //trigger reset for 20 us
    wr_val = 1;
    memwrite(DELAY_BASE_ADDR + DATA_LD_OFF + 4 * i, &wr_val, 1);
    usleep(20);
    wr_val = 0;
    memwrite(DELAY_BASE_ADDR + DATA_LD_OFF + 4 * i, &wr_val, 1);
    rd_val = 0;
    ...
    //assert desired value is set
    rd_val = 0;
    memread(DELAY_BASE_ADDR + DATA_OUT_OFF + 4 * i, &rd_val, 1);
    if (rd_val != taps)
    {
